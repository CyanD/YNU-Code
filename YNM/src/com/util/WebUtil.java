...
public class WebUtil {
	/**
	 * 把form表单提交的数据封装到FromBean中去
	 */
	public static <T> T request2Bean(String objName,
			HttpServletRequest request, Class<T> beanCalss) {
		...
		try {
			...
			while (e.hasMoreElements()) {
				try {
					...
					if (objName != null) {
						name = name.replace(objName, "");
					}
					if(strs!=null&&strs.length>0){
						for(int i=0;i<strs.length;i++){
							strs[i] = StringUtil.GetAndFilt(strs[i]);
						}
					}
					
					BeanUtils.setProperty(bean, name, strs);
				} catch (Exception ef) {
					ef.printStackTrace();
				}
			}
	...
	public static String getPV(String pName,	HttpServletRequest request) {
		// ConvertUtils.register(new SystemPrivilegePKConverter(),
		// SystemPrivilegePK.class);
		try {
