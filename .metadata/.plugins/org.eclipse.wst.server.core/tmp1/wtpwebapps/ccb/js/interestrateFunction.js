$(document).ready(function(){
	...
	//			  var dd1=dd[0].child;
				  ...
				  var li=document.getElementById("inte");
				  //alert(li.innerHTML);
...
$(document).ready(function(){
	$.ajaxSetup({cache:false});
	        ...
	        //json文件位置
	//       url:"data/user.json",
	         url:"../../ccb/sys/findInterestrate.do",
	        //返回数据格式为json
	        dataType: "json",
	        //请求成功完成后要执行的方法
	        success: function(data){
				...
				c="<thead><tr><th>种类</th><th>币别</th><th>期限</th><th>正常利率</th><th>逾期利率</th><th>改动日期</th><th>操作</th></tr></thead>";
				var abc="";
				abc="<tbody>";
				for(var i = 0; i < data1.length; i++) { //循环后台传过来的Json数组  
   					  var datas = data1[i];
   					  ...
   					  if(datas.changedate!=null){
   					      var date=formartDate("yyyy-MM-dd",datas.changedate.time);
   					  }else{
   						  var date="";
   						  
   					  }
   					  aa+= "<tr><td>" + datas.categorys + "</td>"+ "<td>" + datas.moneyType + "</td>"+ "<td>" + datas.deadline + "</td><td>" + datas.interestrate + "</td>"+ "<td>" + datas.overdueInterest + "</td>"+ "<td>" + date + "</td>"+ 
...
//   alert(fm[0].innerHTML);
 ...
 //   alert("123");
       ...
       id.setAttribute("name","position.id");
...
//    alert(fm[0].innerHTML);
    ...
    $.ajaxSetup({cache:false});
	        ...
	        //json文件位置
	         url:"../../ccb/sys/editInterestrate.do?interestrate.id="+id,
	        //返回数据格式为json
	        dataType: "json",
	        //请求成功完成后要执行的方法
	        success: function(data){
	       ...
	       // var date=formartDate("yyyy-MM-dd",datas.changedate.time);
   					//  alert(datas.id);
	                $("#id").val(datas.id);
	                $("#CATEGORYS").val(datas.categorys);
   					$("#MONEYTYPE").val(datas.moneyType);
   					$("#DEADLINE").val(datas.deadline);
   					$("#interestrate").val(datas.interestrate);
   					$("#overdueInterest").val(datas.overdueInterest);
   					$("#changedate").val(datas.changedate);
...
//		 alert("params:"+params);
		...
		$.ajax({
					...
					url : "../../ccb/sys/addOrUpdateInterestrate.do",					
...
//		    alert(id);
	$.ajaxSetup({cache:false});
	        ...
	        //json文件位置
	         url:"../../ccb/sys/removeInterestrate.do?interestrate.id="+id,
	        //返回数据格式为json
	        dataType: "json",
	        //请求成功完成后要执行的方法
...
//							//location.reload('');
							alert('保存成功');
							location.href="interestrateResult.jsp";
...
$(document).ready(function(){
	...
	$('#frm1').validate({
    rules: {
    	"interestrate.categorys": {
        minlength: 1,
        required: true
      },
      "interestrate.moneyType": {
	        minlength: 1,
	        required: true
	  },
	  ...
	  "interestrate.overdueInterest": {
		    minlength: 1,
			required: true
	   },
	   "interestrate.changedate": {
		  date:true,
			required: true
		}
    },
